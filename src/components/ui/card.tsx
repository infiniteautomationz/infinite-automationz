...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn('portal-surface text-[var(--ia-text)] rounded-[var(--ia-radius-md)] border border-[var(--ia-border)]', className)}
    {...props}
  />
));
...
const CardTitle = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h3 ref={ref} className={cn('text-xl md:text-2xl font-bold leading-none tracking-tight text-[var(--ia-text-strong)]', className)} {...props} />
  ),
);
